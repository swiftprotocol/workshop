...
const ToToken = ({
  ...
  toItem: dataType | undefined
  setToItem: (value: dataType) => void
  setTokenOutputValue: (value: string) => void
}) => {
...
const Swap = ({
  ...
  const [tokenOutputValue, setTokenOutputValue] = useState('')
  ...
  return (
    <Stack spacing={6} w="full" p={{ base: 4, sm: 6 }}>
      ...
      <ToToken
        ...
        toItem={toItem}
        setToItem={setToItem}
        setTokenOutputValue={setTokenOutputValue}
      />
      <Rate
        fromItem={fromItem}
        toItem={toItem}
      ...
      <Button
        onClick={() => {
          // Check that everything has a proper value before swapping
          if (!fromItem || !toItem) return
          if (!tokenInputValue || !tokenOutputValue) return
          if (
            parseInt(tokenInputValue) === 0 ||
            parseInt(tokenOutputValue) === 0
          )
            return

          onSwap({
            swapFrom: {
              name: fromItem?.label || 'Token',
              amount: parseInt(tokenInputValue) || 0,
            },
            swapInto: {
              name: toItem?.label || 'Token',
              amount: parseInt(tokenOutputValue) || 0,
            },
          })
        }}
        h={{ base: 12, md: 16 }}
